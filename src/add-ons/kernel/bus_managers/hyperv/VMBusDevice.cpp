...
status_t
VMBusDevice::WritePacket(uint16 type, const void* buffer, uint32 length,
	bool responseRequired, uint64 transactionID)
{
	TRACE_TX("Channel %u TX pkt %u len 0x%X resp %u tran %lu\n",fChannelID,
		type, length, responseRequired, transactionID);
...
status_t
VMBusDevice::ReadPacket(vmbus_pkt_header* _header, uint32* _headerLength,

...
// 5 second timeout for synchronous actions
#define HV_HID_TIMEOUT_US					HV_MS_TO_US(5000)

typedef struct { // HID descriptor
	uint8	length;
	uint8	type;
	uint16	version;
	uint8	country_code;
...
typedef struct { // HID device info
	uint32	length;
	uint16	vendor_id;
	uint16	product_id;
	uint16	version;
...
typedef struct { // HID message header
	uint32	type;
	uint32	length;
} _PACKED hv_hid_msg_header;

typedef struct { // HID protocol request message sent to Hyper-V
	hv_hid_msg_header	header;

	uint32	version;
} _PACKED hv_hid_msg_protocol_request;

typedef struct { // HID protocol response message received from Hyper-V
	hv_hid_msg_header	header;
...
typedef struct { // HID initial device info message received from Hyper-V
	hv_hid_msg_header	header;
...
typedef struct { // HID initial device info acknowledgement message sent to Hyper-V
	hv_hid_msg_header	header;

	uint8	reserved;
} _PACKED hv_hid_msg_initial_dev_info_ack;

typedef struct { // HID input report message received from Hyper-V
	hv_hid_msg_header	header;

	uint8	data[];
} _PACKED hv_hid_msg_input_report;

typedef struct { // HID pipe message header
	uint32	type;
	uint32	length;
} _PACKED hv_hid_pipe_msg_header;

typedef struct { // HID pipe message sent to Hyper-V
...
typedef struct { // HID pipe message received from Hyper-V
	hv_hid_pipe_msg_header	pipe_header;

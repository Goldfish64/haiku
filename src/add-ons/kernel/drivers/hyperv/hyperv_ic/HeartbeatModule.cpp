...
hyperv_heartbeat_supports_device(device_node* parent)
{
	...
	const char* type;
	if (gDeviceManager->get_attr_string(parent, HYPERV_DEVICE_TYPE_ITEM,
			&type, false) != B_OK)
		return 0.0f;
...
hyperv_heartbeat_register_device(device_node* parent)
{
	...
	return gDeviceManager->register_node(parent, HYPERV_HEARTBEAT_DRIVER_MODULE_NAME,
...
driver_module_info gICHeartbeatDriverModule = {
	{
